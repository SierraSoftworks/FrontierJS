...
function AuthMiddleware(application) {
	...
	return function(req, res, next) {
		...
		var params = _.map(req.query, function(value, key) {
							return key + '=' + value;
						}).join('&');

		params = (params ? '?' : '') + params;
		params = params + (params ? '&' : '?') + 'uid=' + authorization.user;

		var hash = crypto.createHash('sha512')
					.update(application.privatekey)
					.update(authorization.expires.getTime())
		...
		if(application.sessionCookie)
			res.cookie(application.sessionCookie, authorization.sessionkey, { expires: authorization.expires.getTime(), httpOnly: true });
		next();
	};
}
