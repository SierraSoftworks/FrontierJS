...
function AuthMiddleware(application) {
	...
	return function(req, res, next) {
		if(!req.query.uid) {
			debug.warn('Failed due to missing UID');
			return next();
		}
		if(!req.query.ts) {
			debug.warn('Failed due to missing timestamp');
			return next();
		}
		if(!req.query.s) {
			debug.warn('Failed due to missing hash');
			return next();
		}
		...
		try {
			...
			debug.warn('Failed due to badly formatted timestamp');
			return next();
		}
		...
		var hash = crypto.createHash('sha512')
					...
					.update(application.callback).update('?').update(params)
					.digest('hex');

		if(hash != authorization.sessionkey) {
			debug.warn('Failed due to hashes not matching');
			return next();
		}
